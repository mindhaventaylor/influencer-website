...
// Get API base URL - use external API for static builds
const getApiBaseUrl = () => {
  if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_API_BASE_URL) {
    return process.env.NEXT_PUBLIC_API_BASE_URL;
  }
  return '/api'; // Default to local API for development
};
...
const api = {
  ...
  async postMessage(influencerId: string, content: string, userId: string) {
    ...
    const response = await fetch(`${getApiBaseUrl()}/post-message`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  async initializeConversation(influencerId: string) {
    ...
    const response = await fetch(`${getApiBaseUrl()}/conversation/initialize`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${session.access_token}`,
  ...
  async createConversationForUser(userId: string) {
    ...
    const response = await fetch(`${getApiBaseUrl()}/conversation/create-for-user`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${session.access_token}`,
